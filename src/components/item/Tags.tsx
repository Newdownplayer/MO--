    ...
    setup: (props, context) => {
        ...
        return () => <>
            <div class={s.tags_wrapper} onTouchmove={onTouchMove}>
                ...
                {tags.value.map(tag =>
                    <div class={[s.tag, props.selected === tag.id ? s.selected : '']} onClick={() => onSelect(tag)} onTouchstart={(e) => onTouchStart(e, tag)} onTouchend={onTouchEnd}>
                        <div class={s.sign}>
                            {tag.sign}
                        </div>
                        <div class={s.name}>
            ...
            <div class={s.more}>
                {hasMore.value ? <Button class={s.loadMore} onClick={fetchTags}>加载下一页</Button> : null}
            </div>
        </>
    }
})